            ...
            {patients.map((p) => (
              <tr key={p.id} className="hover:bg-surface-2/50 transition-colors duration-200 group">
                <td className="px-5 py-3">
                  ...
                  <div className="text-[11px] font-medium text-text-muted mt-0.5">{p.id} &bull; {p.age}y</div>
                </td>
                <td className="px-5 py-3 text-text-main font-medium text-sm">{p.village}</td>
                <td className="px-5 py-3">
                  <div className="text-text-main text-xs">BP: <span className="font-semibold tracking-tight">{p.vitals.bp}</span></div>
                  <div className="text-[11px] text-text-muted mt-0.5">Sugar: <span className="font-semibold text-text-main tracking-tight">{p.vitals.sugar}</span></div>
                </td>
                <td className="px-5 py-3">
                  <RiskBadge risk={p.risk} />
                </td>
                <td className="px-5 py-3 text-text-main text-sm font-medium">{p.ashaWorker}</td>
                <td className="px-5 py-3">
